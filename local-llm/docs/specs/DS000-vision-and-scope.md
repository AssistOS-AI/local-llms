...
| vLLM `0.30.0` | Installable on demand from the image's runner lock (DS004); running models on it is not supported yet: it reads Hugging Face snapshots, which the catalog does not offer yet (runners plan, Phases R3 and R5). |
